...
exports.getAllRecipes = (req, res, next) => {
  try {
    ...
    const { q, region, village, category, festival, ingredient, difficulty, vegetarian, season } = req.query;
    if (q) {
      const lower = q.toLowerCase();
      results = results.filter(r => r.title.toLowerCase().includes(lower) || r.village.toLowerCase().includes(lower) || r.region.toLowerCase().includes(lower) || r.ingredients.some(i => i.toLowerCase().includes(lower)));
    }
    ...
    if (ingredient) results = results.filter(r => r.ingredients.includes(ingredient));
    if (difficulty) results = results.filter(r => r.difficultyLevel === difficulty);
    if (vegetarian) results = results.filter(r => String(r.isVegetarian) === vegetarian);
...
exports.getRecipesByFestival = (req, res, next) => {
  try {
    ...
    const results = store.recipes.filter(r => r.festivalAssociation === festival);
    res.json(results);
  } catch (err) { next(err); }
};
...
exports.getRecipesByIngredient = (req, res, next) => {
  try {
    ...
    const results = store.recipes.filter(r => r.ingredients.includes(ingredient));
    res.json(results);
  } catch (err) { next(err); }
};
...
exports.compareRegions = (req, res, next) => {
  try {
    ...
    if (!regionA || !regionB) return res.status(400).json({ message: 'Both regionA and regionB are required' });
